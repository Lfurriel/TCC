...
import { breakpointOptions, BASE_URL } from './config.js';
import { gerarEmail, gerarCPF, gerarCNPJ, getRandomElement } from './utils.js';

export { breakpointOptions as options };
...
export default function () {
    ...
    try {
        ...
        const totalFlows = randomIntBetween(2, 4);
...
function accessHomePage() {
    const homeRes = http.get(`${BASE_URL}/home/amazon`);

    // Verifica resposta e atualiza métricas
    if (!checkResponse(homeRes, 'Home Page'))
        return null;
...
function executeProductNavigation(homeData) {
    const option = randomIntBetween(1, 4);
...
function navigateOfferPages() {
    const totalPages = randomIntBetween(1, 3);
    let foundProducts = [];
...
function navigateCategoryPages(homeData) {
    ...
    for (let page = 1; page <= totalPages; page++) {
        ...
        // Verifica resposta e atualiza métricas
        if (!checkResponse(categoriaRes, `Categoria página ${page}`)) {
            continue;
        }
...
function searchProductsByName() {
    ...
    try {
        ...
        if (!Array.isArray(produtos) || produtos.length === 0) {
            console.warn(`Nenhum produto encontrado para: ${nomeProduto}`);
            return [];
        }
...
function viewProduct(sku) {
    ...
    return checkResponse(produtoRes, `Visualizar produto ${sku}`);
}
...
function performLogin() {
    ...
    try {
        ...
        return (loginData.data && loginData.data.token) || loginData.token;
    } catch (e) {
        console.warn(`Login - Erro ao parsear JSON: ${e.message}`);
        return null;
    }
}

function performRegistration() {
    const tipoPessoa = Math.random() < 0.7 ? 'PF' : 'PJ';
    ...
    if (tipoPessoa === 'PF') {
        ...
        payload = {
            tipoPessoa: 'PJ',
            cnpj: gerarCNPJ(),
            razaoSocial: `Empresa ${randomIntBetween(1000, 9999)} Ltda`,
            nome: `Empresa ${randomIntBetween(1000, 9999)}`,
            dataNascimento: '1990-01-01',
    ...
    try {
        ...
        return (cadastroData.data && cadastroData.data.token) || cadastroData.token;
    } catch (e) {
        console.warn(`Cadastro - Erro ao parsear JSON: ${e.message}`);
        return null;
    }
}
